	...
	$.fn.emojis = function (params) {
		...
		this.each(function (n, input) {
			...
			function clickEmoji(ev) {
				...
				var d = c + ev.currentTarget.innerHTML;
				$('.note-editable').empty();
				console.log(d);
				$('.note-editable').html(d);
			    $('#summernote').text(d);
			    $('#summernote1').val(d);
			}
			
			// var $button = $("<span>").html(settings.button).css({cursor: 'pointer', 'font-size': settings.fontSize}).on('click', showEmoji);
			var $list = $('<div class="emoji-custom-class" id="summernote-custom-emoji" >').css(defaults.listCSS).css(settings.listCSS);
